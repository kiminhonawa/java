...
public class ListMain02 {
	...
	public static void main(String[] args) {
		...
		// List의 원소 타입은 자바의 기본 타입을 사용할 수 없음. Ex. List<int> X
		// 기본 타입 대신에 wrapper 클래스를 사용해야 함. Ex. List<Interger> O
		// LinkedList와 ArrayList는 원소 추가(add)/삭제(remove)/검색(get) 메서드 이름과 사용법이 동일.

		// 리스트 numbers에 [0, 100) (0이상 100 미만) 범위의 정수 난수 20개를 저장.
		...
		for (int num : numbers) { //리스트 numbers의 원소들을 처음부터 끝까지 순회하면서
			if (num % 2 == 1) {
				odds.add(num);
			}
		}
