...
//-클래스를 작성할 때 생성자를 하나도 정의(선언)하지 않으면 자바 컴파일러가 "기본 생성자"를 자동으로 만들어 줌.
//-클래스를 작성할 때 생성자를 1개 이상을 정의하면, 자바 컴파일러는 기본 생성자를 자동으로 만들지 않음.
//-생성자의 이름은 반드시 클래스 이름과 동일해야 함. 
//-생성자는 리턴 타입을 사용하지 않음. (void도 사용하지 않음.)

//기본 생성자: 아규먼트를 갖지 않는 생성자.
//-일반적으로 필드의 값들을 필드 타입의 기본값으로 초기화 함.
...
public class User {
	...
	// constructor
	public User() {
		//코드를 작성하지 않아도, 각 필드들을 그 타입에 기본값으로 초기화를 해줌.
	}
