...
/*
 * 변수 선언 위치:
 * 1. 필드: 클래스의 멤버로 선언하는 변수. (멤버변수)
 *  - 인스턴스 변수: static이 아닌 필드.
 *  	- 객체를 생성한 후에, 그 참조 변수로 접급해서 사용하는 변수.
 *  - 클래스 변수: static으로 선언된 필드.
 *  	- 프로그램이 시작될 때, 클래스 로더에 의해서 미리 로딩되는 변수.
 * 
 * 2. 지역 변수
 ...
 *  
 *  3. 익명 (내부) 클래스(annonymous class): 이름이 없는 내부 클래스.
 *   	- 선언과 동시에 객체 생성까지 해야만 하는 클래스.
 *  
 *  
 */

public class InnerMain01 {
	public static final int MAX = 5; // 클래스(static) 변수
...
//		static int y;
	...
	//	Enclosing.Nested nested1 = new Enclosing.Nested(2); // import 문 없을때
		nested.printInfo();
