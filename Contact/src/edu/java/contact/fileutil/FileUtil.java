...
public class FileUtil {
	...
	/*
	 * 연락처 데이터 파일을 저장하는 폴더가 존재하지 않으면 생성하고, File 객체를 리턴. 이미 폴더가 생성되어 있는 경우에는, 그 폴더의
	 * File 객체를 리턴.
	 * 
	 * @return 데이터 파일을 저장할 폴더의 File 객체
	 */
	public static File initDataDir() {
		File dir = new File(DATA_DIR); // 객체를 생성
	...
	/*
	 * readDataFromFile. argument로 전달된 File 객체를 사용해서, 파일에 저장된 연락처 정보를 읽고, 그 결과를
	 * List<Contact> 타입의 객체로 리턴.
	 * 
	 ...
	 * 리턴.
	 * 
	 * @return Contact 타입을 원소로 갖는 리스트를 리턴.
	 */
